...
class RouterGenerator:
    ...
    def generate(self, config: RouterConfig) -> str:
        ...
        lines.extend([
            ...
            '    state.path = next; state.params = resolved.params || {}; state.query = parseQuery(encodedQuery); state.route = resolved.route || null; state.fullPath = next + (encodedQuery ? `?${encodedQuery}` : ""); pathSignal.set(state.path); paramsSignal.set(state.params); querySignal.set(state.query); routeSignal.set(state.route); notify(); return true;',
            '  };',
