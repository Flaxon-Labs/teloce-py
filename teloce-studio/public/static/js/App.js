...
const __component = {
  ...
  data() {
    return { project: { id: '', name: 'My Flaxon App', pages: [{ id: 'home', name: 'Home', path: '/' }], theme: { primary: '#6c63ff' } }, status: 'Loading workspace…', device: 'desktop', inspector: 'design', selected: 'Hero section', diagnostics: false, diagnosticMessage: 'No diagnostics yet.' };
  },

  methods: {
    async load() {
      try { const response = await fetch('/api/projects'); const data = await response.json(); if (data.projects && data.projects.length) { const loaded = await fetch('/api/projects/' + data.projects[0].id); this.project = (await loaded.json()).project } else { await this.save() }; this.status = 'Saved locally' } catch (_) { this.status = 'Offline workspace' }
    },
    async save() {
    ...
    async createProject() {
      this.project = { id: '', name: 'New Flaxon App', pages: [{ id: 'home', name: 'Home', path: '/' }], theme: { primary: '#6c63ff' } }; await this.save()
    },
    async generate() {
    ...
    addComponent() {
      this.selected = 'New component'; this.status = 'Component selected'
    },
    selectElement(value) {
      this.selected = value; this.inspector = 'design'
    },
    preview() {
      this.status = 'Preview uses generated files'
    },
    showDiagnostics() {
      this.diagnostics = true
    },
    exportProject() {
      this.diagnostics = true; this.diagnosticMessage = 'Project export is queued for the next implementation slice.'
    },
  },
...
const __initialData = { project: { id: '', name: 'My Flaxon App', pages: [{ id: 'home', name: 'Home', path: '/' }], theme: { primary: '#6c63ff' } }, status: 'Loading workspace…', device: 'desktop', inspector: 'design', selected: 'Hero section', diagnostics: false, diagnosticMessage: 'No diagnostics yet.' };
