...
class RouterGenerator:
    ...
    def generate(self, config: RouterConfig) -> str:
        ...
        lines.extend([
            ...
            '      if (found) return { route, params: Object.fromEntries(compiled.names.map((name, index) => [name, found[index + 1] == null ? undefined : decode(found[index + 1])])) };',
            ...
            '  const navigate = async (target, redirects = new Set()) => { if (destroyed) return false;',
            ...
            '    if (resolved?.route.redirect) { if (redirects.has(next)) throw new Error(`Router redirect cycle detected at ${next}`); redirects.add(next); return navigate(resolved.route.redirect, redirects); }',
            '    for (const guard of guards) { const decision = await guard({ path: next, params: resolved.params, query: parseQuery(encodedQuery), route: resolved.route }, { path: state.path, params: state.params, query: state.query, route: state.route }); if (decision === false) return false; if (typeof decision === "string") return navigate(decision, redirects); }',
            ...
            '  const changeUrl = (target, replace) => { const descriptor = typeof target === "string" ? { path: target } : (target || {}); const path = String(descriptor.path || "/"); const base = normalize(options.base || "/"); const urlPath = base === "/" ? path : `${base}/${path.replace(/^\\//, "")}`; const url = urlPath + (descriptor.query ? queryString(descriptor.query) : ""); if (options.mode === "history") (replace ? history.replaceState : history.pushState).call(history, {}, "", url); else location.hash = urlPath + (descriptor.query ? queryString(descriptor.query) : ""); };',
        ...
        lines.append("const router = createRouter(routes, {")
        self.indent_level += 1
